...
# Development Process
   ...
   * ### Testing, Evaluation and Iteration
      ...
      * I found that my LEFT and RIGHT controls were reversed, this was due to the motors being wired on the opposite side, to explain this im going to give an example, imagine pressing forward and both go, but if you wanted only the left motor to go forward, rather than the left one activating it would instead be the right motor, this means that while both forward and backward commands work as intended, the turning is reversed, to fix this I simply reversed the pins declarations in the code to avoid having to tamper with the wiring.
